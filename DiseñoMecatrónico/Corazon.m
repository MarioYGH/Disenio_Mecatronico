...
% Parámetros de la trayectoria deseada (corazón)
t = linspace(0, 2 * pi, 100);  % Puntos en el rango [0, 2π] para formar el corazón
cx = 2;  % Ajuste de posición en x
cy = 8;  % Ajuste de posición en y

% Generar los puntos de la forma de corazón
Px = 0.15*(cx + 16 * sin(t).^3)+2;
Py = 0.15*(cy + (13 * cos(t) - 5 * cos(2 * t) - 2 * cos(3 * t) - cos(4 * t)))+4;
...
L2 = 4; L3 = 7; L4 = 6; L5 = 4; 
O2O5 = 5;  % Distancia entre los puntos O2 y O5 (puntos fijos en la base)
...
for i = 1:numel(Px)
    ...
    figure(1)
    plot(Px, Py, '-g'), hold on        % Trazo de la trayectoria en forma de corazón
    ...
    hold off
    xlim([-20 20]), ylim([-10 25])  % Límites de los ejes para mejor visualización
